...
На реальном ролике детекция клавиатуры срабатывает, канал подсветки даёт правдоподобные моменты начала, а канал падающих блоков пока не работает — подробный разбор отказов в `mir/vision/README.md`. Числовые пороги настроены по двум источникам данных и требуют проверки на наборе роликов (задача #13).

Самый быстрый способ увидеть это своими глазами (Windows):

```bash
run demo
```

`run.cmd` сам находит подходящий интерпретатор: на машине обычно несколько установок Python, и `python` в PATH указывает не на ту, куда pip ставил пакеты. Разбираться в этом пользователь не обязан.

На других системах то же самое запускается напрямую:
