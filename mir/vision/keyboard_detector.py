...
_MIN_VISIBLE_KEYS = 8
"""Сколько белых клавиш должно поместиться в кадр как минимум.

Задаёт верхнюю границу шага решётки: октава с запасом. Меньше — это уже
не клавиатура, а крупный план нескольких клавиш, и по такому фрагменту
узор всё равно не привязать к абсолютным нотам.
"""

_HARMONIC_TOLERANCE = 0.9
...
class KeyboardDetector:
    ...
    def _estimate_period(self, profile: Frame) -> tuple[float, float]:
        ...
        Профиль стыков — почти периодическая волна со всплеском на каждой
        границе белых клавиш. Её частота находится как сильнейшая
        составляющая спектра в диапазоне допустимых ширин клавиши, а фаза —
        как аргумент той же составляющей. Отдельные пропущенные или лишние
        всплески на оценку почти не влияют: они размазываются по спектру.
        """
        # Медленный ход яркости — виньетка, тень от корпуса, неравномерная
        # подсветка — даёт на низких частотах составляющую сильнее полезной.
        # Без вычитания тренда шаг решётки на реальном ролике определялся
        # в семь раз больше настоящего.
        span = max(len(profile) // 8 | 1, 9)
        trend = cv2.blur(profile.reshape(1, -1).astype(np.float32), (span, 1)).ravel()
        centered = profile - trend

        spectrum = np.fft.rfft(centered)
        freqs = np.fft.rfftfreq(len(centered))

        min_period = max(self.config.min_key_width_px, 4.0)
        max_period = len(centered) / _MIN_VISIBLE_KEYS
        ...
        # Профиль стыков близок к гребёнке узких импульсов, а у неё вторая
        # и третья гармоники почти не уступают основной. Поэтому сильнейшая
        # составляющая проверяется на то, не является ли она сама гармоникой:
        # если на вдвое или втрое меньшей частоте амплитуда сопоставима,
        # основная там.
        for divisor in (4, 3, 2):
            candidate = index // divisor
            if index % divisor or candidate < 1 or not usable[candidate]:
                continue
            if magnitudes[candidate] >= magnitudes[index] * _HARMONIC_TOLERANCE:
                index = candidate
                break
