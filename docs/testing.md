...
107 тестов, из них 98 модульных и 9 интеграционных.

| Файл | Тестов | Что проверяет |
|---|---|---|
| `test_sources.py` | 40 | Распознавание площадок, нормализация ссылок, отклонение неподдерживаемых источников |
| `test_config.py` | 18 | Загрузка настроек, приоритет источников, валидация инвариантов |
| `test_cache.py` | 12 | Кэш: попадания, вытеснение по объёму, устойчивость к повреждённому индексу |
| `test_types.py` | 28 | Модель данных, границы диапазонов, переводы времени |
| `test_ingest.py` | 9 | Полный этап загрузки на реальных файлах через FFmpeg |
...
| `test_hysteresis_enforced` | `off_threshold ≥ on_threshold` — подсветка клавиш будет дребезжать на этапе 3 |
